...
from GeekMusic import YouTube, app
from GeekMusic.core.call import Geek
from GeekMusic.misc import SUDOERS, db
from GeekMusic.utils.database import (is_active_chat,
...
from GeekMusic.utils.formatters import seconds_to_min
from GeekMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from GeekMusic.utils.stream.autoclear import auto_clean
from GeekMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Geek.pause_stream(chat_id)
        ...
        await Geek.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Geek.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Geek.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Geek.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Geek.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
